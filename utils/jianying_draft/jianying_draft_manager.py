...
def _probe_video_meta(video_path: Path) -> Dict[str, Any]:
    try:
        ...
        if os.name == "nt":
            ...
            result = subprocess.check_output(cmd, stderr=subprocess.STDOUT)  # noqa: subprocess-window
...
def _probe_audio_duration(audio_path: Path) -> float:
    try:
        ...
        if os.name == "nt":
            ...
            out_a = subprocess.check_output(cmd_a, stderr=subprocess.STDOUT)  # noqa: subprocess-window
        ...
        if os.name == "nt":
            ...
            out_f = subprocess.check_output(cmd_f, stderr=subprocess.STDOUT)  # noqa: subprocess-window
...
class JianyingDraftManager:
    ...
    def _copy_unique(src: Path, dst_dir: Path) -> Path:
        ...
        ext = src.suffix
        # Windows：整段路径超过 MAX_PATH 时，copy2 在创建目标时可能报 FileNotFoundError（非目录缺失）
        max_path = 240 if os.name == "nt" else 4090

        def pick_dst(preferred: str) -> Path:
            candidate = dst_dir / preferred
            if len(str(candidate)) <= max_path:
                return candidate
            return dst_dir / f"v_{uuid.uuid4().hex}{ext}"

        dst = pick_dst(src.name)
        if dst.exists():
            dst = pick_dst(f"{src.stem}_{uuid.uuid4().hex[:6]}{ext}")
    ...
    async def generate_draft_folder(project_id: str, task_id: str, cancel_event: Optional[asyncio.Event] = None) -> DraftGenerateFolderResult:
        ...
        try:
            ...
            ost_skipped: Set[int] = set()
            ost_start_override: Dict[int, float] = {}

            for idx, seg in enumerate(segments, start=1):
                if cancel_event and cancel_event.is_set():
                    raise asyncio.CancelledError()
                if idx in ost_skipped:
                    continue
                st = float(seg.get("start_time") or 0.0)
                et = float(seg.get("end_time") or 0.0)
                if idx in ost_start_override:
                    st = float(ost_start_override[idx])
                if et <= st:
                    continue
                dur = max(0.0, et - st)
                text = str(seg.get("text") or "").strip()
