...
from clone_narration_video.utils.ffmpeg_utils import probe_duration, resolve_ffmpeg_bin
...
async def ffprobe_get_duration(path: str) -> float:
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, lambda: probe_duration(path))
                ...
                try:
                    ...
                    try:
                        ...
                        with open(str(out_path), "wb") as f:
                            f.write(audio_data)
                    dur = await ffprobe_get_duration(str(out_path))
                    if delete_after:
                        try:
                            if out_path.exists():
                                out_path.unlink()
